...
public interface IBrandDao extends JpaRepository<BrandEntity, Long> {
	...
	/*
	 * @Query(value = "SELECT  COUNT(p.brandEntity.brandId)" +
	 * " FROM ProductEntity p RIGHT JOIN BrandEntity  b ON p.brandEntity.brandId = b.brandId"
	 * + " GROUP BY b.brandId") Page<BrandEntity>
	 * countQuantityProductOfBrand(Pageable pageable);
	 */
}
